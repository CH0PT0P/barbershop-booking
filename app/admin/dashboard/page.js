...
export default function Dashboard() {
  ...
  // New booking state
  const [bookingStep, setBookingStep] = useState('search') // search | newclient | datetime | confirm
  ...
  const [bookNextStep, setBookNextStep] = useState('datetime')
  ...
  async function fetchClients() {
    const { data, error } = await supabase
      .from('clients')
      .select(`*, appointments (id, date, time, service, status)`)
  ...
  // Create or find client, then book appointment
  ...
  // New Booking tab — confirm booking
    ...
    return (
      <div className="bg-zinc-800 rounded-xl p-4">
        <div className="flex justify-between items-center mb-4">
          <button
            onClick={() => { const d = new Date(calMonthState); d.setMonth(d.getMonth() - 1); setCalMonthState(d) }}
          ...
          return (
            <button
              ...
              className={`rounded-xl py-2 text-xs font-medium transition border ${
                ...
                isBooked ? 'border-zinc-800 text-zinc-700 cursor-not-allowed' :
                'border-zinc-600 hover:border-white text-white'
              }`}
            >
  ...
  // Booked times for selected book date
  const bookedTimesForDate = appointments
    .filter(a => a.date === bookDate && a.status === 'booked')
    .map(a => a.time)

  const bookedTimesForNextDate = appointments
    .filter(a => a.date === bookNextDate && a.status === 'booked')
    .map(a => a.time)

  // Client search results
  const searchResults = clientSearch.length > 1
    ? clients.filter(c =>
            ...
            <div className="relative flex" style={{ height: `${TIMELINE_HEIGHT}px` }}>
              <div className="w-16 flex-shrink-0 relative">
                {Array.from({ length: DAY_END - DAY_START + 1 }).map((_, i) => {
                  ...
                  return (
                    <div key={hour} className="absolute text-xs text-gray-500" style={{ top: `${topPercent}%`, transform: 'translateY(-50%)' }}>
                      {label}
                    </div>
                  )
                })}
          ...
          <div>
            {bookingSuccess ? (
              ...
              <div className="flex flex-col gap-6">
                ...
                {(bookingStep === 'search' || bookingStep === 'newclient') && (
                  <div>
                    ...
                    <input
                      ...
                      value={clientSearch}
                      onChange={(e) => { setClientSearch(e.target.value); setBookingStep('search'); setSelectedClient(null) }}
                    ...
                    {searchResults.length > 0 && !selectedClient && (
                      <div className="flex flex-col gap-2 mb-3">
                        {searchResults.map(client => (
                          <button
                            key={client.id}
                            onClick={() => { setSelectedClient(client); setClientSearch(client.name) }}
                    ...
                    <button
                      onClick={() => setBookingStep('newclient')}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {services.map(s => (
                      <button
                        ...
                        className={`rounded-xl py-3 px-4 text-sm font-medium transition border ${
                          selectedService === s
                ...
                <button
                  ...
                  disabled={!bookDate || !bookTime || !selectedService}
                ...
                return (
                  <div key={client.id} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-5">
                    ...
                    <div className="flex gap-4 mt-3">
                      <span className="text-xs text-gray-500">{totalVisits} completed visit{totalVisits !== 1 ? 's' : ''}</span>
                      <span className="text-xs text-gray-500">{upcoming} upcoming</span>
                    </div>
                  </div>
                )
              })
                ...
                {availability.map((day) => (
                  <div key={day.id} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        onClick={() => updateAvailability(day.id, { is_blocked: !day.is_blocked })}
                        className={`text-xs px-3 py-1 rounded-full transition ${
                          day.is_blocked ? 'bg-zinc-700 text-gray-400' : 'bg-green-900 text-green-400'
                        }`}
                      >
                    ...
                    {!day.is_blocked && (
                      <div className="flex gap-3 mt-3">
                        <div className="flex flex-col gap-1 flex-1">
                          ...
                          <input
                            type="time"
                            defaultValue={day.start_time?.slice(0, 5)}
                        ...
                        <div className="flex flex-col gap-1 flex-1">
                          ...
                          <input
                            type="time"
                            defaultValue={day.end_time?.slice(0, 5)}
              ...
              <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 mb-4">
                ...
                <div className="grid grid-cols-7 gap-1 mb-4">
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        onClick={() => setOverrideDate(dateStr)}
                        className={`rounded-lg py-2 text-sm font-medium transition relative ${
                          isSelected ? 'bg-white text-black' :
                          hasOverride ? 'bg-zinc-700 text-white' :
                ...
                {overrideDate && (
                  <div className="flex flex-col gap-3 border-t border-zinc-700 pt-4">
                    ...
                    <button onClick={addOverride} className="w-full bg-white text-black py-2 rounded-full text-sm font-semibold hover:bg-gray-200 transition">
                      Save Change
                    </button>
                  </div>
                )}
                    ...
                    <div key={o.id} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          {o.is_blocked ? 'Blocked off' : `${o.start_time?.slice(0, 5)} – ${o.end_time?.slice(0, 5)}`}
                        </p>
                      </div>
                      <button onClick={() => deleteOverride(o.id)} className="text-red-400 hover:text-red-300 text-sm transition">
                        Remove
                      </button>
                    </div>
                  ))
      ...
      {selectedAppt && !showBookNext && (
        <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50" style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0}} onClick={() => setSelectedAppt(null)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-t-3xl w-full p-6 pb-10 min-h-[67vh] max-h-[80vh] overflow-y-auto" style={{position: 'relative', zIndex: 9999}} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => setShowBookNext(true)}
                className="w-full bg-white text-black py-3 rounded-full font-semibold transition hover:bg-gray-200"
              >
      ...
      {selectedAppt && showBookNext && (
        <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50" onClick={() => { setShowBookNext(false) }}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-t-3xl w-full max-w-lg p-6 pb-10 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              onClick={confirmBookNext}
              disabled={!bookNextDate || !bookNextTime}
